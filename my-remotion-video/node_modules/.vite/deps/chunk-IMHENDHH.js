...
var t2length = ({
  ...
  let error = 1;
  let t = length2 / totalLength;
  let step = (length2 - func(t)) / totalLength;
  let numIterations = 0;
  while (error > 1e-3) {
    const increasedTLength = func(t + step);
    const increasedTError = Math.abs(length2 - increasedTLength) / totalLength;
    if (increasedTError < error) {
      error = increasedTError;
      t += step;
    } else {
      const decreasedTLength = func(t - step);
      const decreasedTError = Math.abs(length2 - decreasedTLength) / totalLength;
...
var makeQuadratic = ({
  ...
  const b = { x: cpx, y: cpy };
  const c = { x, y };
  const length2 = getQuadraticArcLength([a.x, b.x, c.x, 0], [a.y, b.y, c.y, 0], 1);
  const getTotalLength = () => {
    return length2;
  };
  const getPointAtLength2 = (len) => {
  ...
  return {
    getPointAtLength: getPointAtLength2,
    getTangentAtLength: getTangentAtLength2,
    getTotalLength,
    getC,
...
var makeCubic = ({
  ...
  const d = { x, y };
  const length2 = getCubicArcLength({
    sx: [a.x, b.x, c.x, d.x],
    sy: [a.y, b.y, c.y, d.y],
    t: 1
  });
  ...
  for (let i = 0; i < instructions.length; i++) {
    ...
    if (instruction.type === "L") {
      if (segments.length > 0) {
        const length2 = Math.sqrt((currentPoint.x - instruction.x) ** 2 + (currentPoint.y - instruction.y) ** 2);
    ...
    if (instruction.type === "Z") {
      if (segments.length > 0) {
        const length2 = Math.sqrt((segments[segments.length - 1].startPoint.x - currentPoint.x) ** 2 + (segments[segments.length - 1].startPoint.y - currentPoint.y) ** 2);
        segments[segments.length - 1].instructionsAndInfo.push({
          instruction,
          function: makeLinearPosition({
            x0: currentPoint.x,
    ...
    if (instruction.type === "C") {
      ...
      const length2 = curve.getTotalLength();
      if (segments.length > 0) {
        segments[segments.length - 1].instructionsAndInfo.push({
          instruction,
          length: length2,
...
var CBEZIER_MINMAX_EPSILON = 1e-8;
function minmaxQ(A) {
...
var makeArc = ({
  ...
  const getPointAtLength2 = (fractionLength) => {
    if (fractionLength < 0) {
      fractionLength = 0;
  ...
  return {
    ...
    getTangentAtLength: (fractionLength) => {
      ...
      const p1 = getPointAtLength2(fractionLength);
      let p2;
      if (fractionLength < 0) {
        fractionLength = 0;
      } else if (fractionLength > length2) {
      ...
      if (fractionLength < length2 - point_dist) {
        p2 = getPointAtLength2(fractionLength + point_dist);
      } else {
        p2 = getPointAtLength2(fractionLength - point_dist);
      }
...
function splitCurveAsPoints(points, segmentCount = 2) {
  ...
  for (let i = 0; i < segmentCount - 1; i++) {
    ...
    const split2 = decasteljau(remainingCurve, tRelative);
...
function extendInstruction(commandsToExtend, referenceCommands) {
  ...
  const countPointsPerSegment = new Array(numReferenceSegments).fill(void 0).reduce((accum, _d, i) => {
...
var warpPath = (path, transformer, options) => {
  ...
  const interpolated = svgPathInterpolate(withZFix, (options == null ? void 0 : options.interpolationThreshold) ?? getDefaultInterpolationThreshold(withZFix));
...
//# sourceMappingURL=chunk-IMHENDHH.js.map
