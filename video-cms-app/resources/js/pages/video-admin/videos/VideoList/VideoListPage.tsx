                            ...
                            videos.map((video) => (
                                ...
                                >
                                    ...
                                    <td className="space-x-2 px-4 py-4 text-center whitespace-nowrap">
                                        {/* Status: pending or failed -> GENERATE button */}
                                        {video.status === 'pending' ||
                                        video.status === 'failed' ? (
                                            <Button
                                                asChild
                                                variant="outline"
                                                className="border-green-600 text-xs hover:bg-green-50"
                                            >
                                                <Link
                                                    href={route(
                                                        'video.generate',
                                                    ...
                                                    method="post" // Triggers the VideoController@generate method
                                                    as="button"
                                                >
                                                    Generate Video
                                                </Link>
                                            ...
                                            // Status: processing -> Loading indicator
                                            <span className="animate-pulse text-xs font-medium text-yellow-600 dark:text-yellow-400">
                                                Processing...
                                            </span>
                                        ) : (
                                            // Status: completed -> Download link
                                            <Button
                                                asChild
                                                variant="default"
                                                className="bg-green-600 text-xs hover:bg-green-700"
                                            >
                                                {/* Link uses the output_path saved by the Job */}
                                                <a
                                                    href={`/storage/${video.output_path}`}
                                                    target="_blank"
                                                >
                                                    Download MP4
                                                </a>
